    ...
    while True:
        try:
            ...
            if super_match(play_coordinate_dict, 'confirm_button'):
                ...
                if not super_rgb_match(play_coordinate_dict, 'player_one_blue_bet_label'):
                    betsize = get_betsize()
            ...
            # 遊戲結束
            elif super_match(play_coordinate_dict, 'dealer_21_label') & (super_rgb_match(play_coordinate_dict, 'dealer_card_three')):
                log_info('dealer blackjack')
                sleep(3.5)
                get_all_hand_img(play_coordinate_dict, True)
                get_all_hand_value(True)
                # count_win_loss()
                sleep(5)
            elif super_match(play_coordinate_dict, 'dealer_21_label'):
                log_info('dealer_21_label')
                sleep(3.5)
                get_all_hand_img(play_coordinate_dict, False)
                get_all_hand_value(False)
                # count_win_loss()
                sleep(5)
            elif super_match(play_coordinate_dict, 'player_21_label') & (super_rgb_match(play_coordinate_dict, 'player_one_layer_one_card_three') | super_rgb_match(play_coordinate_dict, 'player_one_layer_one_card_three_1')) & (super_match(play_coordinate_dict, 'dealer_score_green_label') | super_match(play_coordinate_dict, 'dealer_score_grey_label')):
            ...
            if super_match(play_coordinate_dict, 'dont_buy_insurance_button'):
                decide_to_buy_insurance(play_coordinate_dict)
        except Exception:
