...
def point_count(br, bet, player: Player, dealer: Player, player_win_count, player_lose_count,total_water):
    ...
    print('br',round(br, 2))
    print('total_water',round(total_water, 2))
    print('win_loss_without_water',round(br + total_water,2))
...
def start(player_win_count, player_lose_count, br ,total_water):
    ...
    while (len(deck.cards) > 156):
        bet = 1
        old_win_count = player_win_count
        old_lose_count = player_lose_count
        # 這邊會不會跟著變動
        current_hi_low = deck.hi_low_avg
        if current_hi_low >=3:
            bet = 1
        player_list = []
        dealer = Player()
